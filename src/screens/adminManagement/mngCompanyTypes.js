...
const pageTitle = 'ادارة انواع الشركات';

const  MngCompanyTypes = ()=> {
    ...
    const handleTableChange = (pagination, filters, sorter) => {
      ...
      const getCompanyTypes = (current=1,pageSize=25)=>{
        ...
        var requestOptions = {
          ...
          fetch(`https://haitham-chamber-of-commerce.herokuapp.com/api/company-types?sort[0]=id:desc`, requestOptions)
      ...
      const addCompanyType = (data)=>{
        ...
        addDataWithToken(`company-types`,data,
        (err,result)=>{
          setAddLoading(false);
          const {error} = result
          if(err){
           ...
           if(!error){
            message?.success('تم اضافة النوع بنجاح');
            getCompanyTypes();
            addCompanyTypeForm?.resetFields();
           }else{
            console.log(error)
      ...
      function showDeleteConfirm(A_field_name,id) {
        confirm({
            ...
            onOk() {
                setAddLoading(true);
                deleteDataWithToken(`company-types/${id}`,(err, result)=>{
                    ...
                    if(err){
                        ...
                        if(!error){
                            message?.success('تم حذف النوع بنجاح');
                            getCompanyTypes();
                        }else{
                            console.log(error)
                            message?.error(error?.message)
        ...
        const editCompanyType = (data)=>{
            ...
            setAddLoading(true);
            editDataWithToken(`company-types/${selectedCompanyType?.id}`,data,(err, result)=>{
                const {error} = result
                if(err){
                    message?.error('خطأ في الشبكة')
                }else{
                    if(!error){
                        message?.success('تم تعديل النوع بنجاح');
                        getCompanyTypes();
                    }else{
                        console.log(error)
                        message?.error(error?.message)
    ...
    return(
        <Layout className='mainContainer'>
                ...
                >
                    ...
                    <Form.Item  
                        ...
                        rules={[
                            {
                              required: true,
                              message: `يرجى كتابة اسم النوع`,
                            }
                          ]}
                        >
                             <Input size="large" placeholder={'اسم النوع'} style={{minWidth:300}}  />
                    </Form.Item>
            ...
            <GlobalSider currentPage={'companytypes'} currentSub={'sub1'} />
            <Layout
                style={{
                padding: '0 24px 24px',
                marginTop:75
            ...
            >
                ...
                <Col md={24} >
                    ...
                    >
                        ...
                        <Column title="اسم نوع الشركة"  width="300px" key="name" fixed="left" dataIndex="name" />
...
export default MngCompanyTypes
